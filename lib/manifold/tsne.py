...
from .metric import compute_gaussian_similarity
from .base_class import ManifoldBase
...
class TSNE(ManifoldBase):
    ...
    def fit(self, data):
        ...
        Returns:
              numpy.ndarray(numpy.float): Data in embedded space of shape [n_samples, self._n_components]
        ...
        asym_prob_table_old_space = _compute_asym_prob_table(similarity_function=compute_gaussian_similarity,
    ...
    def _gradient_descent(self,
        ...
        Args:
            asym_prop_old_space (numpy.ndarray(numpy.float)): Table of size [n_samples, n_samples], where each cell
                shows the asymmetic probabilty between to samples (indices refer to what samples) of the orignal data.
            data (numpy.ndarray(numpy.float)): Data of new space. Data should have shape of [n_samples, n_features].
            neighbours_old_space (numpy.ndarray(numpy.float): Idx in shape [n_samples, n_neighbours] to the neighbours,
                for calculation example refer to from sklearn.neighbors import NearestNeighbors.

        Returns:
            numpy.ndarray(numpy.float): Data embedded in new space. Data shape is [n_samples, self._n_components]
        ...
        asym_prob_new_space = _compute_asym_prob_table(similarity_function=compute_gaussian_similarity,
                                                       neighbour_distances=neighbour_distances,
...
def _compute_asym_prob_table(neighbour_distances, neighbour_idx, similarity_function=compute_gaussian_similarity):
    """Compute asymetric probability table.

    For details refer to the T-SNE paper.

    Args:
        neighbour_distances (numpy.ndarray(numpy.float)): Distances in shape [n_samples, n_neighbours]. For calculation
            example refer to from sklearn.neighbors import NearestNeighbors.
        neighbour_idx (numpy.ndarray(numpy.float)): Idx in shape [n_samples, n_neighbours] to the neighbours, for
            example calculation refer to from sklearn.neighbors import NearestNeighbors.
        similarity_function (function): Function to call calculate the distance from. For example refer to
            manifold.metric.compute_gaussian_similarity().

    Returns:
        numpy.ndarray(numpy.float): Table of size [n_samples, n_samples], where each cell shows the asymmetic probabilty
            between to samples (indices refer to what samples).
...
def _compute_dissim_table(neighbour_idx, neighbour_distances, similarity_function=compute_gaussian_similarity):
    ...
    Args:
        neighbour_distances (numpy.ndarray(numpy.float)): Refer to manifold.tsne._compute_asym_prob_table() docstring.
        neighbour_idx(numpy.ndarray (numpy.float)):  Refer to manifold.tsne._compute_asym_prob_table() docstring.
        similarity_function (function):  Refer to manifold.tsne._compute_asym_prob_table() docstring.

    Returns:
        numpy.ndarray(numpy.float): Table of size [n_samples, n_samples], where each cell shows the dissimilarity
            between to samples (indices refer to what samples).
